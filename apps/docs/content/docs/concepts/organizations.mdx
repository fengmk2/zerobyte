...
<Callout type="info">
	Zerobyte also has an instance-wide user role named `admin` for global tasks such as registration control and user
	management. That global role is separate from organization roles like `member`, `admin`, and `owner`.
</Callout>
...
<Callout type="warn">
	Only add users to an organization if you are comfortable with them managing that organization's backup resources and
	using the storage, volume, and notification capabilities configured for the Zerobyte instance.
</Callout>
