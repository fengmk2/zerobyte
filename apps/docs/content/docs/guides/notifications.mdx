...
<Callout type="info">Create destinations once, then reuse them across as many backup schedules as you want.</Callout>
...
<Callout type="warn">
	Outbound HTTP notification destinations must use an origin listed in `WEBHOOK_ALLOWED_ORIGINS`. This applies to
	Generic Webhook, Gotify, self-hosted ntfy servers, and Custom Shoutrrr URLs that resolve to a custom HTTP endpoint.
</Callout>
