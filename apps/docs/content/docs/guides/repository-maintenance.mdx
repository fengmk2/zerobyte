...
<Callout type="warn">
	Doctor is helpful, but it is lighter than a full offline integrity scrub. Today it does not read every stored data
	pack and it does not prune repository data. Treat `healthy` as a useful application-level signal, not as proof that
	every blob was recently re-read.
</Callout>
