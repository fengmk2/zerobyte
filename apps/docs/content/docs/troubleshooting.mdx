...
<Callout type="warn">
	Never share sensitive information (passwords, access keys, personal data) in public issue reports. Redact them from
	logs before posting.
</Callout>
...
<Callout type="warn">
	**Last resort**: If nothing else works, you can run with `privileged: true`. This disables most container isolation
	and should only be used temporarily for diagnosis.
</Callout>
